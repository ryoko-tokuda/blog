...
<!-- more -->
## 目次
- [はじめに：アクセス許可と Azure ロール](#はじめにアクセス許可と-Azure-ロール)
  - [アクセス許可とは](#アクセス許可とは)
...
Azure RBAC では、ユーザーやグループに特定の Azure ロールを割り当てることで、リソースに対するアクセス許可を制御します。  
つまり、"Azure ロール (*1)" はひとことで表すと「ユーザーやグループに一括で付与できるアクセス許可のまとまり」です。  
...
### カスタム ロールが必要になるシナリオ
カスタム ロールを使用することで、Log Analytics ワークスペースにデータを送信しているリソース レベル (*2) や、ワークスペース内のテーブル単位 ([後述のアクセス許可の説明](#Log-Analytics-に関連するアクセス許可)をご確認ください) で、ワークスペース内に収集されたログへのアクセス許可を制御することができます。  

(* 2) リソース レベルでのアクセス許可が必要になるのは、リソース コンテキストで Log Analytics ワークスペースにアクセスしており、かつそのワークスペースのアクセス制御モード (*3) が "リソースまたはワークスペースのアクセス制御" の場合です。  
この場合は、データが収集されている Log Analytics ワークスペースに対するアクセス許可ではなく、データ収集元であるリソース (VM など) に対してアクセス許可をもっている必要があります。  

(*3) アクセス制御モードの詳細については以下のブログにてご紹介しておりますため、こちらをご参照ください。  
- [Log Analytics ワークスペースのアクセス制御モードの違いについて](https://jpazmon-integ.github.io/blog/LogAnalytics/AccessControlMode/)  


### Log Analytics に関連するアクセス許可
カスタム ロールとして、以下のアクセス許可を含めることで、Log Analytics ワークスペース内のデータに対して、ログ送信元のリソースや、ワークスペース内のテーブル レベルでのアクセス制御が可能です。

`Microsoft.Insights/logs/*/read` :  
リソースのすべてのログ データを表示します。  

`Microsoft.Insights/logs/<tableName>/read` :  
リソースからのデータが収集される特定のテーブルを表示するための、レガシのアクセス許可です。 
これを使用することで、ワークスペース内のテーブル レベルでのアクセス制御が可能です。
例) `Microsoft.Insights/logs/Heartbeat/read` 
テーブル レベルの読み取りアクセスを設定する新しい方法については、以下弊社公開情報をご参照ください。
[Log Analytics ワークスペースでテーブル レベルの読み取りアクセスを管理する](https://learn.microsoft.com/ja-jp/azure/azure-monitor/logs/manage-table-access)
...
4. [アクセス許可] タブで [アクセス許可の追加] を押下します。  
   ...
   ロールに追加したいアクセス許可にチェックを入れます。  
   (*5) リソース プロバイダーの検索文字が残ったままの場合、アクセス許可が表示されない場合があります。  
      必要なアクセス許可が表示されない場合は、一度検索欄を空白にしてください。
5. アクセス許可の選択が完了したら、[追加] を押下します。
6. 上記手順の 6 と同様です。
7. 上記手順の 7 と同様です。  
...
1. Azure portal でロールを割り当てたいスコープのページを開きます。
...
本記事が、アクセスの管理方法の理解やご要件に沿った適切なアクセス管理の一助となれば幸いです。